...
class my_install_data(_install_data):
    ...
    def run(self):
        ...
        2/ parse each file in the to_be_parsed_files list:
            - search each line with one of these patterns:
                ALIGNAKETC
                ALIGNAKVAR
                ALIGNAKBIN
                ALIGNAKRUN
...
alignak_modules_path = os.path.join(
    alignak_cfg['ALIGNAKETC'], 'arbiter', 'modules'
)
...
for subdir, dirs, files in os.walk(manifest["__pkg_name__"]):
    for file in files:
        ...
        if subdir and 'ALIGNAKETC' in subdir:
            data_files.append(
                (os.path.join(
                    alignak_etc_path,
                    re.sub(
        ...
        # Other files are ignored by this process
        else:
            print("Ignoring file: %s" % os.path.join(subdir, file))
